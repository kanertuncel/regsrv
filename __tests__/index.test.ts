...
import whojs from '../src/index.js';
...
describe('whojs', () => {
  ...
  it('throws for invalid domain', async () => {
    await expect(whojs('not a domain')).rejects.toThrow('Invalid domain or URL provided.');
  });

  it('throws for unsupported TLD', async () => {
    await expect(whojs('example.unknown')).rejects.toThrow('Unsupported TLD: .unknown. No RDAP server found.');
  });
  ...
  it('returns standardized data for a valid domain', async () => {
    ...
    const data = await whojs('example.com');
  ...
  it('throws for 404 response', async () => {
    ...
    await expect(whojs('notfound.com')).rejects.toThrow('Domain not found: notfound.com');
  });
  ...
  it('throws for other HTTP errors', async () => {
    ...
    await expect(whojs('error.com')).rejects.toThrow('RDAP lookup failed for error.com: 500 Internal Server Error');
  });
}); 